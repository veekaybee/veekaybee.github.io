...
date = '2022-12-22'
site = '@vboykis'
title = 'Welcome to the jungle, we've got fun and frames Working with medium-ish data in Pandas'
description = 'Continuing working with Viberary'
+++
...
The easiest way to look at data at a glance is still Pandas DataFrames, so I thought that if I took the complication of the cloud out of this, I could read this data into a local Pandas DataFrame to perform some summary stats on it. 
...
Or so I thought. When I tried to use [`read_json`](https://pandas.pydata.org/docs/reference/api/pandas.read_json.html) in Pandas, the kernel took over 2 minutes just to read the data in, but I didn't understand what was going on, or how I could fix it. 

To understand the performance constraints, I had to go pretty deep into the Pandas ecosystem. 
...
This is an enormous pain, and it saves some space in the in-memory DataFrame object but the resulting object is enormous ,which ends up being over 6GB as-is. If you recall, our original file was 9GB.  
...
Reading from parquet directly also immediately resulted in a dead kernel
...
Since I'd rather troubleshoot data and model issues at this point, I'm leaving polars to try for another time. 
...
In my case, it was important for me to go through this Journey so I could better understand all the internal components. But it was nice not having to do any of it.

It easily reads in 2.3 million rows and performs aggregations. 

It easily infers the correct schema (although, in this case, it was from a CSV representation of my original JSON file, which I created also through Spark.)

<img width="543" alt="Screen Shot 2023-01-17 at 4 21 05 PM" src="https://user-images.githubusercontent.com/3837836/213014843-24eaf9c8-278b-43f3-a47b-26d75ae2a899.png">
