...
1. They type of request. `do_GET` and `do_POST` [handle this implicitly](https://stackoverflow.com/a/50944691) in the [HTTP implementation](# see https://docs.python.org/3/library/http.server.html#http.server.SimpleHTTPRequestHandler.do_GET).
...
In development, FastAPI uses [`uvicorn`](https://www.uvicorn.org/)(https://asgi.readthedocs.io/en/latest/), an [ASGI server]  to listen for incoming requests and handle them according to the routes defined in your application. Uvicorn initializes the ASGI server, binds it to a [socket connections](https://github.com/encode/uvicorn/blob/ae8253f10b9f73f10e92be52a0d9f70696b77c62/uvicorn/server.py#L115 on [port `8000`.](https://github.com/encode/uvicorn/blob/ae8253f10b9f73f10e92be52a0d9f70696b77c62/uvicorn/main.py#L73), and starts listening for incoming connections. So, when I send a `GET` request to the main route hosted by default on port 8000, I expect to get back `ciao mondo!` as a response. 
...
8. The `.get` [method on `APIRouter` takes the path](https://github.com/fastapi/fastapi/blob/144f09ea146b2cc026bf317f730aa0e0dbc3de24/fastapi/routing.py#L1366) and retunrs `return self.api_route`. This is the point where the decorater is actually called - [you can see the decorator in that method](https://github.com/fastapi/fastapi/blob/144f09ea146b2cc026bf317f730aa0e0dbc3de24/fastapi/routing.py#L963) takes a `DecoratedCallable` function as input and returns a decorated `add_api_route`, which actually [appends the route to the list of routes.](https://github.com/fastapi/fastapi/blob/144f09ea146b2cc026bf317f730aa0e0dbc3de24/fastapi/routing.py#L961) 
