...
When you first run `llama.cpp`, it gives you a very long log. 

<script src="https://gist.github.com/veekaybee/5236546a1b09fe9f03c42525871cf126.js"></script>

To understand this output, I started reading the codebase, and ran straight into GGUF. [GGUF (GPT-Generated Unified Format)](https://github.com/ggerganov/ggml/blob/master/docs/gguf.md) is the file format used to serve models on `llama.cpp` and other local runners like [Llamafile, Ollama and GPT4All.](https://semaphoreci.com/blog/local-llm) 

Before we get too far down this path, let's first take a deep dive into how machine learning models work and what artifacts they produce. 
...
Since most models are trained on PyTorch these days, artifacts follow PyTorch's method for serializing objects to disk.
...
- **The Output**: Generally for text completion, the output is a probability distribution for each position in the input. For chat completions, the model returns the likelihood that any given word completes a phrase, and if it's served as a product, it just returns the text output based on the highest probabilities, with numerous strategies for [how that text is selected.](https://huggingface.co/blog/how-to-generate)

{{< figure  width="600" src="https://github.com/veekaybee/veekaybee.github.io/assets/3837836/cb311adb-79f3-4eea-85be-7329e4aeb111">}}

In short, we have inputs that we convert to outputs. In addition to the model's output, we also have the model itself that is generated as an artifact of the modeling process.
...
What's actually in that model artifact? Let's go simpler and build a small linear regression model in PyTorch to find out. Anything can be anything else in machine learning, and it turns out that luckily for us, [linear regression is also](https://d2l.ai/chapter_linear-regression/index.html) a (shallow) neural network, so we can work with it in PyTorch and map our simple model to more complex ones using the same framework. 
...
For example, let's say that we produce [artisinal hazlenut spread.](https://www.greatitalianchefs.com/features/hazelnuts-piedmont), and want to predict how many jars of Nocciolatella we'll produce on any given day. Let's say we have some data available to us, and that is, how many hours of sunshine we have per day, and how many jars of Nocciolatella we've been able to produce every day.   

It turns out that we feel more inspired to produce hazlenut spread when it's sunny out, and we can clearly see this relationship between input and output in our data (we do not produce Nocciolatella Fri-Sunday because we prefer to spend those days writing about data serialization formats): 
...
In the specific case of linear regression, there are no true hyperparameters, although we can plausibly consider the learning rate we set in PyTorch to be one.
...
This tells us that the output, y (the number of jars of Nocciolatella), can be predicted by one input variable, `x` (hours of sunshine) with its given weights, `β`  plus an error term `ε`, or the distance between each data point and the regression linegenerated by the equation.

Our task is to continuously solve this equation for the difference between our actual Y as presented by our data and a predicted Y based on the algorithm to find the smallest sum of squared differences between each point and the line. In other words, we'd like to minimize the error, because it will mean that, at each point, our predicted y is as close to our actual y as we can get it, given the other points.
...
Let's add our actual data into the model by initializing both X and y as [PyTorch Tensor objects](https://pytorch.org/docs/stable/tensors.html).  
...
Within code, our input data is `X`, which is a torch tensor object, and our output data is `y`. We initialize a LinearRegression which subclasses the PyTorch Module, with one linear layer, which has one input feature (sunshine) and one output feature (jars of Nocciolatella).
...
class LinearRegression(nn.Module):
    ...
    def forward(self, x):
        return self.linear(x)
```
We then pick how we'd like to optimize the results of the model, aka how its loss should converge. In this case, we start with `mean squared error`, and then modify it to use `RMSE`, the square root of the average squared difference between the predicted values and the actual values in a dataset. 
...
The optimizer, as we can see, has its own state dict, which consists of these hyperparameters we discussed before: the learning rate, the weight decay, and more:
...
Now that we have a trained model object, we can pass in new values, for example we can pass in an X value of `5` days of sunshine and see how many jars of Nocciotella we expect to make. We do this by passing in `5` to the instantiated model object, which is now a combination of the method used to run the linear regression equation and our state dict, the weights, the current set of weights and biases to give a new predicted value. We get 9, which pretty close to what we'd expect.
...
As machine learning with deep learning models trained with PyTorch exploded, these security issues came to a head, and in 2021, Trail of Bits released a post on [Fickling, about the insecurity of pickle files.](https://github.com/trailofbits/fickling) 

Engineers started developing a library known as [safetensors](https://github.com/huggingface/safetensors/tree/main) as an alternative to pickle. Safetensors was a [format developed at HuggingFace](https://github.com/huggingface/safetensors/discussions/111) that was not only more efficient, but, as the same, says, safer and more ergonomic. 
...
After a concerted push from both Trail of Bits and EleutherAI, a security audit of the format was conducted and found satisfactory, [which led to HuggingFace adapting it as the default format for models on the Hub.](https://huggingface.co/blog/safetensors-security-audit) going forward. (Big thanks to [Stella and Suha](https://twitter.com/vboykis/status/1759268551129452654) for this history, and everyone who contributed to this thread.)

## How safetensors works

How do safetensors work? As with most things in LLMs at the bleeding edge, the code and commit history will do most of the talking. [Let's take a look at the file spec.](https://github.com/huggingface/safetensors#yet-another-format-) 
...
This is different than `state_dict` and `pickle` files, but follows the natural evolution from Python objects, to full-fledged file format. 

A file is a way of storing our data generated from programming language objects, in bytes on disk. 
In looking at different file format specs ([Arrow](https://arrow.apache.org/docs/format/CDataInterface.html),[Parquet](https://parquet.apache.org/docs/file-format/), [protobuf](https://protobuf.dev/)), we'll start to notice some patterns around how they're laid out. 
...
GGML was initialy both a library and a complementary format created specifically for on-edge inference for whisper. You can also [perform fine-tuning](https://www.reddit.com/r/LocalLLaMA/comments/15y9m64/fine_tuningggml_quantiziation_on_apple_silicon/) with it, but generally it's used to read models trained on PyTorch in GPU Linux-based environments and converted to GGML to run on Silicon. 
